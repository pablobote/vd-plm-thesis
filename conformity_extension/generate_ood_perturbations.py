#!/usr/bin/env python3
"""
generate_ood_perturbations.py

Transforms the out-of-distribution vulnerability-detection benchmark
(BigVul/DiverseVul-schema JSONL, with columns like func / target /
project / cwe / commit_id / commit_date, released with the "Revisiting
Pre-trained Language Models for Vulnerability Detection" artifact) into
the peer-pressure perturbation format used by the "Easier to Mislead
Than to Correct" study design (RQ1 consensus x authority, RQ2a
committed-peer-count sweep, RQ2b authority-label-count sweep).

Run `python generate_ood_perturbations.py --help` for all options.
...
11. dataset_name DEFAULTS TO "revisitvd_ood" -- this script is for the
    out-of-distribution benchmark released with the "Revisiting
    Pre-trained Language Models for Vulnerability Detection" artifact
    (not self-collected). The row schema ("big_vul_idx" + "dataset":
    "other" fields) happens to match a DiverseVul-style export, but
    that's a schema fingerprint, not the actual source. Override with
    --dataset_name if pointing this at a different input file --
    it's just a label, doesn't affect generation.
...
def main():
    ...
    p.add_argument("--dataset_name", default="revisitvd_ood",
    ...
    filenames = {
        "rq1":  "ood_benchmark_rq1_rq3.jsonl",
        "rq2a": "ood_benchmark_rq2a.jsonl",
        "rq2b": "ood_benchmark_rq2b.jsonl",
    }
    ...
    if args.combined:
        ...
        combined_path = combined_dir / "ood_benchmark_all_combined.jsonl"
